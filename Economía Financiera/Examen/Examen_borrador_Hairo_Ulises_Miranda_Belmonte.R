...
renglon.ceros <- function(antes, total) {
  c(rep(0,antes), 1, rep(0,total-antes-1))}
 
alpha = c(1, 2,3) # estos valores son s??lo un ejemplo 
beta = c(1, 2, 3) #estos valores son s??lo un ejemplo 
renglones <- c(alpha*rnorm(1), beta*rnorm(1)^2, #primer rengl??n 
               renglon.ceros(0,6),renglon.ceros(1,6), alpha, beta, renglon.ceros(3, 6),
               renglon.ceros(4,6))
A <- matrix(renglones, byrow = T, nrow = 6)
...
garch_matrix(c(1,1,1),c(1,2,3))

install.packages("purrr")
library(purrr)

matrices <- rerun(50, garch_matrix(alpha, beta))
...
################FIN DEL EJERCICIO DEL PROFESOR#################3
...
matrices <-function(n.sim){
  rerun(n.sim, garch_matrix(alpha, beta))
} 
a<-matrices(10)
prods <- accumulate(a, `%*%`) 
normas <-matrix(map_dbl(prods,norm)) 
A2.m<- normas/ seq_along(normas)
 
rho <- max(abs(eigen(A2.m)$values))
